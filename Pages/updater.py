...
class Page(ETS2LAPage):
    ...
    def render(self):
        ...
        with Container(styles.FlexVertical() + styles.Padding("24px") + styles.Gap("24px") + styles.Margin("30px 60px 60px 60px")):
            if updates == []:
                Text(_("You have a local commit that is waiting to be pushed."), styles.Description() + styles.Classname("text-xs"))
            elif not updates:
                Text(_("No updates available. (It might take up to a minute for the page to update after a new commit)"), styles.Description())
                ...
                with Button(self.update):
                    Text(ngettext(
                        ...
                        len(updates)
                    ).format(count=len(updates)), styles.Classname("font-semibold"))
