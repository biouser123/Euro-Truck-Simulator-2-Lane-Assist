...
class Plugin(ETS2LAPlugin):
    ...
    def layout(self):
        ...
        for widget in widgets:
            if widget.data:
                if widget.element.name in sizes:
                    total_width += sizes[widget.element.name]["width"] * self.widget_scaling
                    total_count += 1
                else:
        ...
        for widget in widgets:
            widget_width = sizes.get(widget.element.name, {"width": 100})["width"] * self.widget_scaling
            ...
            if widget.data:
                offset_x += widget_width + 20 * self.widget_scaling  # add padding between widgets
    ...
    def background(self):
        ...
        return Rectangle(
            Point(-self.middle_pixels - 10 * self.widget_scaling, -10 * self.widget_scaling, anchor=self.anchor),
            Point(-self.middle_pixels + self.total_width + 10 * self.widget_scaling, height, anchor=self.anchor),
            color=Color(0, 0, 0, 0),
